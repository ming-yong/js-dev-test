...
import { User, Repo } from '../typings/Repo';
...
repos.get('/', async (_: Request, res: Response) => {
  ...
  try {
    ...
    //Get local repos
    let rawFileData = fs.readFileSync(LOCAL_FILE_PATH, 'utf8');
    let localRepos = JSON.parse(rawFileData);
    ...
    const ERROR:AppError = {name: "custom error", status: 500, message: "Something went wrong!"};
    result = ERROR;
    console.error(error);
  }
